Rewrite and re-run the existing `create_presentation.py` script to regenerate `trailvolt-presentation.pptx` with the following strict requirements. Overwrite the previous file completely.

**LANGUAGE: All text in the presentation must be in English. No Russian, no mixed language. Every title, bullet, label, quote, and note — English only.**

---

**SLIDE DIMENSIONS:**
Widescreen 16:9 — width: 33.87 cm (13.33 inches), height: 19.05 cm (7.5 inches).

---

**STRICT LAYOUT RULES — apply to every single slide:**

Text zones:
- Left margin from slide edge: minimum 1.8 cm
- Right margin from slide edge: minimum 1.8 cm
- Top margin from slide edge: minimum 1.2 cm
- Bottom margin from slide edge: minimum 1.2 cm
- Title text box: always starts at x=1.8 cm, y=1.0 cm, width must not exceed 55% of slide width when an image is present on the right, or 90% of slide width when there is no image
- Body text box: starts at x=1.8 cm, y=3.2 cm, width same rule as title
- No text box may overflow outside the slide boundary
- All text boxes must have internal padding: left=0.3 cm, right=0.3 cm, top=0.2 cm, bottom=0.2 cm
- Word wrap must be enabled on all text boxes
- Font sizes: Title 32pt bold, Subtitle 20pt, Body bullets 15pt, Small labels 12pt
- If a title is long (more than 45 characters), reduce font size to 26pt automatically
- Never let text overflow its text box — reduce font size before overflowing

Image placement rules:
- Images must ALWAYS maintain their original aspect ratio — never stretch, never squish
- To place an image: calculate the original width/height ratio, then fit the image inside the allocated zone while preserving ratio using `contain` logic: scale down proportionally until both width and height fit within the target zone, then center the image within that zone
- Standard image zone for split-layout slides: x = 57% of slide width, y = 1.2 cm, width = 40% of slide width, height = slide height minus 2.4 cm
- For slides with two images side by side: each image gets 19% of slide width, placed at x=57% and x=78%, y=3.5 cm, height up to 12 cm — both must be proportionally scaled to fit
- For slides with three images in a row (line cards slide): each image gets 28% of slide width, placed at y=10 cm, height up to 7.5 cm — all three proportionally scaled
- For full-bleed cover and final slides: image fills right 45% of the slide from top to bottom, proportionally scaled to cover that zone (crop if needed — use anchor top-left)
- Images must never overlap text boxes
- Images must never overflow the slide boundary

---

**COLORS AND FONTS:**
- Slide background: `#0B1626` (dark navy) — apply to ALL slides
- Title color: `#FFFFFF`
- Body text color: `#E5E7EB`
- Accent / bullet marker color: `#3FFFAB` (electric mint)
- Quote text color: `#3FFFAB`
- Label text (small caps above title): `#3FFFAB`, 11pt, bold
- Fonts: Arial Bold for titles, Arial for body text
- Bullet marker: use the character `▸` in mint color `#3FFFAB` before each bullet, followed by a space, then the bullet text in `#E5E7EB`
- To color only the bullet marker: add the `▸` as a separate run with mint color, then the text as a second run in body color — do this for every bullet point

---

**IMAGE HANDLING:**
- Before inserting any image, check `os.path.exists()` — skip image silently if missing, do not crash
- All `.webp` files must be converted to `.png` using Pillow before inserting: `from PIL import Image`, open the webp, save as png to `tmp_images/` folder, use the png path
- After all slides are created, delete the `tmp_images/` folder
- Implement a helper function `add_image_proportional(slide, image_path, zone_left, zone_top, zone_width, zone_height)` that:
  1. Opens the image with Pillow to get original pixel width and height
  2. Calculates scale = min(zone_width / img_width, zone_height / img_height)
  3. Computes final_width = img_width × scale, final_height = img_height × scale
  4. Centers within zone: offset_x = (zone_width − final_width) / 2, offset_y = (zone_height − final_height) / 2
  5. Places image at (zone_left + offset_x, zone_top + offset_y) with final_width × final_height
  6. This function must be used for EVERY image insertion in the script — no exceptions

---

**SLIDE CONTENT — 15 slides, all text in English:**

**Slide 1 — Cover**
Label: `APRIL 2026 — TRAILVOLT`
Title: `TrailVolt`
Subtitle: `One Pack. Every Horizon.`
Body: `How I used Claude Code + GitHub to build a premium e-commerce site for a concept-driven product — and why a boxed solution was never an option.`
Image: `assets/images/hero-pack-main.webp` — right 45% full-height zone

**Slide 2 — Why Not Shopify or Wix**
Label: `THE TOOL QUESTION`
Title: `A premium product needs a premium digital environment`
Bullets:
...
Image: `CCP/Front Page.png` — right zone

**Slide 3 — The Problem**
Label: `THE ORIGIN`
Title: `Four bags. One person. One endless reorganisation.`
Bullets:
...
Image: `CCP/3 tipe bags.png` — right zone

**Slide 4 — The Idea**
Label: `THE CONCEPT`
Title: `Not a backpack. A system.`
Quote (large, mint): `"1 base pack + 8 snap-on modules = everything you need"`
Bullets:
...
Image: `assets/images/modular-system-diagram.webp` — right zone

**Slide 5 — Three Lines**
Label: `THE ARCHITECTURE`
Title: `Three contexts. One philosophy.`
Three column labels (mint): `EVERYDAY 22L` / `TRAVEL 30L` / `TRAIL 28L`
Three column subtexts:
- City, office, gym, evening / 600D Ballistic Nylon / Core Black
- Airport, 4 nights, no checked bag / IATA-compliant / Slate Grey
- Wicklow, Wild Atlantic Way, Pyrenees / 500D Cordura / Forest Green
Three images bottom row: `assets/images/line-card-everyday.webp`, `assets/images/line-card-travel.webp`, `assets/images/line-card-trail.webp` — proportionally scaled side by side

**Slide 6 — Modular Ecosystem**
Label: `THE MODULES`
Title: `8 modules. One rail. Zero compromise.`
Bullets:
...
Two images: `assets/images/module-phone.webp` and `assets/images/module-tech.webp` — side by side right zone

**Slide 7 — Homepage Strategy**
Label: `UX ARCHITECTURE`
Title: `The homepage is not a storefront. It's a product onboarding.`
Bullets:
...
Image: `CCP/Front Page.png` — right zone, large

**Slide 8 — Configurator**
Label: `KEY UX ELEMENT`
Title: `The configurator is not an order form. It's the system interface.`
Bullets:
...
Two images: `CCP/Configure core pack.png` and `CCP/Configure core back step 3.png` — side by side right zone

**Slide 9 — Modules Page**
Label: `PRODUCT DEPTH`
Title: `Modules are a full product — not an accessories section.`
Bullets:
...
Two images: `assets/images/module-shoe.webp` and `assets/images/module-power.webp` — side by side right zone

**Slide 10 — Founder Story**
Label: `TRUST ARCHITECTURE`
Title: `Built Out of Frustration. Perfected by Obsession.`
Quote (mint, large): `"I wanted one pack that could do everything — not because I was lazy, but because the right system should be that capable."`
Attribution: `— Ruslan, Founder`
Bullets:
...
Two images: `CCP/Founder.png` and `assets/images/about-wicklow.webp` — side by side right zone

**Slide 11 — Community**
Label: `SOCIAL PROOF`
Title: `12,000+ people didn't just buy a backpack — they chose a system.`
Bullets:
...
Two images: `CCP/Community.png` and `CCP/Community 1.png` — side by side right zone

**Slide 12 — Sustainability**
Label: `BRAND VALUES`
Title: `Longevity is not a position. It's a construction parameter.`
Bullets:
...
Two images: `CCP/What Goes Into Every Pack sustainability.png` and `assets/images/sustainability-repair.webp` — side by side right zone

**Slide 13 — Visual Language**
Label: `VISUAL SYSTEM`
Title: `Dark navy. Electric mint. Not sportswear — cinematic.`
Bullets:
...
Two images: `assets/images/pack-everyday-black.webp` and `assets/images/about-airport.webp` — side by side right zone

**Slide 14 — Why Custom**
Label: `TECHNICAL APPROACH`
Title: `Shopify doesn't know what a modular attachment rail is.`
Bullets:
...
Image: `CCP/languege .png` — right zone

**Slide 15 — Final**
Label: `CONCLUSION`
Title: `TrailVolt is not a site about a backpack.`
Subtitle: `A digital environment for a product that requires understanding, not just purchasing.`
Large mint text: `One Pack. Every Horizon.`
Five closing statements:
...
Write the full script as `create_presentation.py` in the project root, then run it immediately.
...
Run the script immediately after writing it with:
```
python create_presentation.py
```

Then confirm the file exists and print its size. If any error occurs, fix it and re-run automatically without asking.
